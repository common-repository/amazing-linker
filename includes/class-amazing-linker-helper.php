...
if( !class_exists( 'Amazing_Linker_Helper' ) ) {

	class Amazing_Linker_Helper {
		...
		public function amazon_price_notice($updated_at,$template=null) {
			
			$settings = (array)get_option('amazing_linker_settings_tab_option');
			$button_text_color = "#ffffff";
			if(array_key_exists('button_text_color', $settings)) {
            	$button_text_color = sanitize_text_field($settings['button_text_color']);
            }
			
			$updated_at = min($updated_at);
			$updated_at = date('Y-m-d H:i', $updated_at);
			/*$site_url = $this->site_url();*/
			if($template == "product_box" || $template == "widget") {
				$notice= '<p class="al-footer-amazon-notice product-box-amazon-notice">Updated at '.$updated_at.' UTC-<span class="al-tooltip" data-tooltip="Product\'s all informations are accurate as of the date/time indicated and are subject to change.">More</span></p>';
			} elseif($template == "popup_box") {
				$notice= '<p class="al-footer-amazon-notice" style="margin-top:10px!important;margin-bottom:0!important;font-size:10px!important;padding-top:0;padding-bottom:0;color:'.$button_text_color.'!important;">Last updated at '.$updated_at.' UTC</p>';
			} else {
				$notice= '<p class="al-footer-amazon-notice product-box-amazon-notice">Last updated at '.$updated_at.' UTC - <span class="al-tooltip" data-tooltip="Product prices and availability are accurate as of the date/time indicated and are subject to change.">More info</span></p>';
			}
			return $notice;
		}
		...
		public function reviews_n_rating($asin) {
			...
			$associate = (array)get_option('amazing_linker_associate_tab_option');
			...
			/*$html = Amazinglinker\Htmldom\file_get_html($url);
		...
		public function amazing_linker_array_sanitize($array) {
		    $newArray = array();
		    if (count($array)) {
			    ...
			    if(!empty($image_set)) {
			    	...
			    	if($first_key == '0') {
			    		if (isset($image_set[0]['HiResImage']['URL'])) {
				    		...
				    		$image_url = AMAZING_LINKER_PLUGIN_URL.'public/images/unavailable.png';
				    	}
			    	} else {
		...
		public function price_to_float($price) {
			...
			$position = strcspn( $price , '0123456789' );
			$currency_symbol = substr($price,0,$position);
			$sale_price = substr($price,$position);
			$sale_price = (float)str_replace(',', '', $sale_price);
			$currency_and_price = [];
			$currency_and_price[] = $currency_symbol;
			$currency_and_price[] = $sale_price;
		...
		public function price_grabber_database ($item) {
			...
			if((!isset($item['Offers']['TotalOffers']) || $item['Offers']['TotalOffers'] == 0) && !isset($item['ItemAttributes']['ListPrice']) && !isset($item['Variations']['Item']) && !isset($item['VariationSummary']) && (!isset($item['OfferSummary']) || $item['OfferSummary']['TotalNew'] == 0)) {
	    		...
	    		if($item['Variations']['TotalVariations'] > 1) {
		    		...
		    		foreach ($item['Variations']['Item'] as $variation_item) {
		    			...
		    			if(isset($variation_item['Offers']) && strpos(strtolower($variation_item['Offers']['Offer']['Merchant']['Name']), 'amazon') !== false && $variation_item['Offers']['Offer']['OfferAttributes']['Condition'] == 'New') {
			    			...
			    			if(isset($variation_item['Offers']['Offer']['OfferListing']['SalePrice'])) {
			    				...
			    				$variation_price = $this->price_to_float($variation_item['Offers']['Offer']['OfferListing']['SalePrice']['FormattedPrice']);
			    				array_push($price, $variation_price[1]);
			    			} else {
			    				$variation_price = $this->price_to_float($variation_item['Offers']['Offer']['OfferListing']['Price']['FormattedPrice']);
			    				array_push($price, $variation_price[1]);
			    			}		
		    		...
		    		$price = $variation_price[0].number_format(min($price),2).'-'.$variation_price[0].number_format(max($price),2);
